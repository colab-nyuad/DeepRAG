...
# --- Reranking based on custom reranker scores ---
def rerank_docs_by_custom_score(
    original_chunks: List[Dict[str, Any]],
    reranked_scores: List[float] # Assumes scores correspond 1:1 with original_chunks
) -> List[str]:
    """Reranks parent docs based on max custom reranker score of their chunks."""

    if len(original_chunks) != len(reranked_scores):
        logging.error(f"Mismatch between number of chunks ({len(original_chunks)}) and scores ({len(reranked_scores)}). Cannot rerank.")
        # Fallback: Return documents based on original chunk order (best effort)
        fallback_ids = []
        seen_fallback = set()
    ...
    for i, chunk in enumerate(original_chunks):
        ...
        if parent_doc_id:
            doc_scores[parent_doc_id] = max(doc_scores.get(parent_doc_id, -float('inf')), score)
    ...
    for query_idx_str in progress_bar:
        ...
        try:
            ...
            if not chunks or not original_query:
                ...
                # Prepare passages (list of texts)
                passages_to_rerank = [chunk.get("text", "") for chunk in chunks]
                
                if not passages_to_rerank:
                    logging.warning(f"Query {query_idx_str}: No passages prepared for custom reranking.")
                    recall_results = calculate_recall_at_k([], ground_truth_ids, k_values_to_calculate)
                else:
                    # Perform reranking using the custom reranker
                    # *** ASSUMPTION: reranker.rerank returns list of scores in order ***
                    reranked_scores = custom_reranker.rerank(query=original_query, passages=passages_to_rerank)
                    
                    # Validate the output format (simple length check)
                    if not isinstance(reranked_scores, list) or len(reranked_scores) != len(chunks):
                         logging.error(f"Query {query_idx_str}: Custom reranker did not return a list of scores matching the number of chunks. Got: {type(reranked_scores)}, Length: {len(reranked_scores) if isinstance(reranked_scores, list) else 'N/A'}")
                         # Fallback: Use original retrieval order
                         fallback_ids = []
                         seen_fallback = set()
                         for ch in chunks:
                              p_id = ch.get('doc_id')
                        ...
                        # Rerank parent documents based on aggregated custom scores
                        reranked_parent_doc_ids = rerank_docs_by_custom_score(chunks, reranked_scores)
